/*
    Modified by github.com/tekert to make it compatible with ahk v2
    Originaly done by: https://github.com/Kalamity/classMemory/

    Below is the original doc (modified most of the error reporting doc, now they are all exceptions)
    --------------------
...
class _ClassMemory
{
    ...
    ; Method:    __new(program, dwDesiredAccess := "", &handle := "", windowMatchMode := 3)
    ...
    findPID(program, windowMatchMode := "3")
    {
        ...
        if windowMatchMode
        {
            ...
            ; remove hex prefix as SetTitleMatchMode will throw a run time error. This will occur if integer mode is set to hex and user passed an int (unquoted)
                        windowMatchMode := StrReplace(windowMatchMode, "0x",,,, 1)
        ...
        if (!pid && RegExMatch(program, "i)\bAHK_EXE\b\s*(.*)", &fileName))
        {
            ...
            {
                ErrorLevel := ProcessExist(fileName)
                pid := ErrorLevel
            }
        }
    ...
    ;                   When an invalid type is passed the method returns NULL and sets this.ErrorLevel to -2 TODO: throws on v2
    ...
    ; Method:   readRaw(address, &buffer, bytes := 4, aOffsets*)
    ...
    readRaw(address, &bufferObj, bytes := 4, aOffsets*)
    {
        ...
        return DllCall("ReadProcessMemory", "Ptr", this.hProcess, "Ptr", aOffsets.Length ? this.getAddressFromOffsets(address, aOffsets*) : address, "Ptr", bufferObj, "UPtr", bytes, "Ptr", this.pNumberOfBytesRead)
    }
    ...
    readString(address, sizeBytes := 0, encoding := "UTF-8", aOffsets*)
    {
        ...
        if !sizeBytes  ; read until null terminator is found or something goes wrong
        {
            ...
            {   ; Lets save a few reads by reading in 4 byte chunks
                try
                {
                    DllCall("ReadProcessMemory", "Ptr", this.hProcess, "Ptr", address + ((outterIndex := A_index) - 1) * 4, "Ptr", bufferObj, "Ptr", 4, "Ptr", this.pNumberOfBytesRead)
                    Loop loopCount
                    {
                        if NumGet(bufferObj, (A_Index - 1) * encodingSize, charType) = 0 ; NULL terminator
                        {
                            if (bufferSize < sizeBytes := outterIndex * 4 - (4 - A_Index * encodingSize))
                                bufferObj.Size := sizeBytes
                            break 2
                        }
                    }
                }
        ...
        if DllCall("ReadProcessMemory", "Ptr", this.hProcess, "Ptr", address, "Ptr", bufferObj, "Ptr", sizeBytes, "Ptr", this.pNumberOfBytesRead)
    ...
    writeString(address, string, encoding := "utf-8", aOffsets*)
    {
        ...
        bufferObj := Buffer(requiredSize)
        StrPut(string, &bufferObj, StrLen(string) + (this.insertNullTerminator ?  1 : 0), encoding)
        return DllCall("WriteProcessMemory", "Ptr", this.hProcess, "Ptr", aOffsets.Length ? this.getAddressFromOffsets(address, aOffsets*) : address, "Ptr", bufferObj, "Ptr", requiredSize, "Ptr", this.pNumberOfBytesWritten)
    }
    ...
    ;                   Valid types are UChar, Char, UShort, Short, UInt, Int, Float, Int64 and Double.
    ;                   Note: Types must not contain spaces i.e. " UInt" or "UInt " will not work.
    ;                   When an invalid type is passed the method returns NULL and sets this.ErrorLevel to -2
    ;       aOffsets* - A variadic list of offsets. When using offsets the address parameter should equal the base address of the pointer.
    ;                   The address (base address) and offsets should point to the memory address which is to be written to.
    ; Return Values:
    ;       Non Zero -  Indicates success.
    ...
    getProcessBaseAddress(windowTitle, windowMatchMode := "3")
    {
        if (windowMatchMode && A_TitleMatchMode != windowMatchMode)
        {
            ...
            windowMatchMode := StrReplace(windowMatchMode, "0x",,,, 1) ; remove hex prefix as SetTitleMatchMode will throw a run time error. This will occur if integer mode is set to hex and matchmode param is passed as an number not a string.
    ...
    ; Method:            getModuleBaseAddress(module := "", &aModuleInfo := "")
    ; Parameters:
    ...
    getModuleBaseAddress(moduleName := "", &aModuleInfo := "")
    {
        ...
        r := this.getModules(&aModules, True)
        if (r < 0)
            return r ; -4, -3
    ...
    ; Method:                   getModuleFromAddress(address, &aModuleInfo)
    ...
    getModuleFromAddress(address, &aModuleInfo, &offsetFromModuleBase := "")
    {
        ...
        for k, module in aModules
        {
            ...
            {
                aModuleInfo := module
                offsetFromModuleBase := address - module.lpBaseOfDll
                return 1
            }
        }
        return -1
    ...
    isTargetProcess64Bit(PID, hProcess := "", currentHandleAccess := "")
    {
        ...
        closeHandle := False
        if !hProcess || !(currentHandleAccess & (_ClassMemory.aRights.PROCESS_QUERY_INFORMATION | _ClassMemory.aRights.PROCESS_QUERY_LIMITED_INFORMATION))
    ...
    */
    ...
    ; Method:               getModules(&aModules, useFileNameAsKey := False)
    ...
    getModules(&aModules, useFileNameAsKey := False)
    {
        ...
        Loop moduleCount
        {
            hModule := NumGet(lphModule, (A_index - 1) * A_PtrSize, "UPtr")
    ...
    getEndAddressOfLastModule(&aModuleInfo := "")
    {
        ...
        if !moduleCount := this.EnumProcessModulesEx(&lphModule)
            return -3
        hModule := NumGet(lphModule, (moduleCount - 1) * A_PtrSize, "UPtr")
    ...
    GetModuleFileNameEx(hModule := 0, fileNameNoPath := False)
    {
        ...
        VarSetStrCapacity(&lpFilename, 2048 * 2) ; V1toV2: or try: lpFilename := Buffer(2048 * 2)
    ...
    GetModuleInformation(hModule, &aModuleInfo)
    {
        ...
        ret := DllCall("psapi\GetModuleInformation"
                    , "Ptr", this.hProcess
                    , "Ptr", hModule
                    , "Ptr", MODULEINFO
                    , "UInt", A_PtrSize * 3)
    ...
    hexStringToPattern(hexString)
    {
        ...
        hexString := StrReplace(hexString, "?", "?",, &numReplacements)
        wildCardCount := numReplacements
    ...
    stringToPattern(string, encoding := "UTF-8", insertNullTerminator := False)
    {
        ...
        bufferObj := Buffer(requiredSize)
        StrPut(string, bufferObj, length + (insertNullTerminator ?  1 : 0), encoding)
        Loop requiredSize
            AOBPattern.Push(NumGet(bufferObj, A_Index-1, "UChar"))
        return AOBPattern
    }
    ...
    ; Method:           rawPatternScan(&buffer, sizeOfBufferBytes := "", aAOBPattern*)
    ...
    ; Parameters:
    ;   bufferObj           The binary Buffer() to be searched.
    ...
    rawPatternScan(&bufferObj, sizeOfBufferBytes := "", startOffset := 0, aAOBPattern*)
    {
        ...
        if (sizeOfBufferBytes + 0 = "" || sizeOfBufferBytes <= 0)
            sizeOfBufferBytes := bufferObj.Size
        if (startOffset + 0 = "" || startOffset < 0)
            startOffset := 0
        return this.bufferScanForMaskedPattern(&bufferObj, sizeOfBufferBytes, &patternMask, &AOBBuffer, startOffset)
    }

    ; Method:           getNeedleFromAOBPattern(&patternMask, &needleBuffer, aAOBPattern*)
    ...
    getNeedleFromAOBPattern(&patternMask, &needleBuffer, aAOBPattern*)
    {
        ...
        for i, v in aAOBPattern
            patternMask .= (v + 0 = "" ? "?" : "x"), NumPut("UChar", round(v), needleBuffer, A_Index - 1)
        return round(aAOBPattern.Length)
    }
    ...
    VirtualQueryEx(address, &aInfo)
    {
        if (aInfo.__Class != "_ClassMemory._MEMORY_BASIC_INFORMATION")
            aInfo := _ClassMemory._MEMORY_BASIC_INFORMATION()

        if (this.hProcess = "")
            throw Error("Process handle is null (unsufficient privilegies to read and write memory addresses, and to perform pattern scans."
               "if the program is run using admin privileges, then this script will also need admin privileges)", -1)
    ...
    */
    ...
    ; Method:               PatternScan(startAddress, sizeOfRegionBytes, patternMask, &needleBuffer)
    ...
    {
        bufferObj := Buffer()
        if !this.readRaw(startAddress, &bufferObj, sizeOfRegionBytes)
            return -1
        if (offset := this.bufferScanForMaskedPattern(&bufferObj, sizeOfRegionBytes, &patternMask, &needleBuffer)) >= 0
    ...
    bufferScanForMaskedPattern(&hayStack, sizeOfHayStackBytes, &patternMask, &needle, startOffset := 0)
    {
        ...
        hayStackAddress := hayStack.Ptr
        needleAddress := needle.Ptr
    ...
    MCode(mcode)
    {
        ...
        ; TODO: for some reason e[m[1]] doesn't work on ahk v2 on may 24, workaround
        ee := m[1] = 1 ? 4 : 1

        if !DllCall("crypt32\CryptStringToBinary", "str", m[3], "uint", 0, "uint", ee, "ptr", 0, "uint*", &s, "ptr", 0, "ptr", 0)
            return
        ...
        if DllCall("crypt32\CryptStringToBinary", "str", m[3], "uint", 0, "uint", ee, "ptr", p, "uint*", &s, "ptr", 0, "ptr", 0)
            return p
    ...
    class _MEMORY_BASIC_INFORMATION
    {
        ...
        __set(key, params, value)
        {
            ...
            if aLookUp.Has(key)
            {
                NumPut(aLookUp[key].Type, value, this.pStructure+0, aLookUp[key].Offset)
                return value
            }
