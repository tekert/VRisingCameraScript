...
class VRising
{
    ...
    _useMemScan := True
    _lockAxysLevel := 0.27
    _scanMenusFunc := ""    ; Important, for SetTimer Off we have to use the same ObjBindMethod that was used to create the timer, since its not static, it creates a new object on each call.
    ...
    __new(processName := "VRising.exe", useMemScan := True, moduleName := "", moduleNameOffset := 0, moduleOffsets := [])
    {
        ...
        ; We need to enable it when the script starts to detect when the process is not open to detect when the process is active or restarted
